...
more, which is the entire reason this source exists today and the other four in the design
spec's table do not:
    ...
    ig_bio        needs Instagram bio text, which does not exist anywhere in this system yet.
                  `social.py`'s docstring is explicit that bios belong to the not-yet-built
                  browser worker and are deliberately never fetched or stored today; a
                  contacts source that read one would be reading a field nothing produces.
...
`website` has none of those problems, so it is the only source built here.
...
#: `contacts.source` for this module. The only one it ever writes.
SOURCE_WEBSITE = "website"

#: Fixed per the source table in the design spec -- `website` is worth 0.8, same order of
#: trust as `review_reply` and ahead of `ig_bio`/`directory`/`search`, none of which this
#: module builds.
CONFIDENCE = 0.8
...
def find_contacts(text: str | None, *, source_url: str | None) -> ContactExtraction:
